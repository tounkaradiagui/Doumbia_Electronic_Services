...
<section class="bg-secondary py-5">
  <div class="container text-center pt-5 mt-lg-4 mt-xl-5">
    <div class="position-relative mx-auto my-3" style="max-width: 850px;">
      <h1 class="display-3 position-relative z-2 mb-0">Explorez un Monde d'Innovation et de Services Exceptionnels</h1>
...
<section class="container py-5 my-sm-2 my-md-4 my-lg-5" id="services">
  <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 gy-3 gy-sm-4 gy-xl-5 gx-4 gx-md-5 pb-xxl-4 mb-sm-2 mb-lg-0 mb-xl-2">
    ...
    <?php
    $query = getServicesData('activites');
    if (mysqli_num_rows($query) > 0) {
      foreach ($query as $visible) {

    ?>
        <div class="col-md-6 mb-3">
          <div class="card shadow h-100 d-flex flex-column">
            <div class="card-header">
              <h6 class="card-title"><?= $visible['title'] ?></h6>
            </div>
            <?php if (!empty($visible['image'])) : ?>
              <img src="./config/images/<?= $visible['image'] ?>" class="card-img-top" alt="Image" />
            <?php else : ?>
              Aucune image
            <?php endif; ?>
            ...
            <div class="card-footer">
              <span class="small text-muted">Date de publication : <?= formatDate($visible['updated_at']) ?></span>
            </div>
  ...
  <div class="swiper-wrapper">
    ...
    <div class="row">
      <div class="col-md-4">
        ...
        <article class="swiper-slide w-sm-auto h-auto shadow">
